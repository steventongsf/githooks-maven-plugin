...
public class TestGitFolderFinder {
    ...
    public void findGitTestFolderCurrentDir() throws IOException {
        GitFolderFinder gff = new GitFolderFinder(targetDir);
        for (String folder: gff.subFolderNames.keySet()) { 
            File file = new File(targetDir.getAbsolutePath()+"/.git.bak/"+folder);
    ...
    public void findGitTestFolderParentDir() throws IOException {
        GitFolderFinder gff = new GitFolderFinder(targetDir);
        for (String folder: gff.subFolderNames.keySet()) { 
            File file = new File(targetDir.getAbsolutePath()+"/.git/"+folder);
            FileUtils.forceMkdir(file);
        }
        assertEquals(new File(targetDir.getCanonicalFile()+"/.git"),gff.find());
    }
}
